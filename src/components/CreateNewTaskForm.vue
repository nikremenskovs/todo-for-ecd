...
import { required, maxLength, helpers, between } from "@vuelidate/validators";
...
export default {
  ...
  data() {
    return {
      today: new Date(),
      twoWeeksFromToday: new Date(Date.now() + 14 * 24 * 60 * 60 * 1000),
      ...
      model: {
        ...
        dueDate: {
          ...
          between: helpers.withMessage(
            "Due date must be from today to two weeks in the future",

            between(
              this.formatDate(this.today),
              this.formatDate(this.twoWeeksFromToday)
            )
          ),
          // between: (value) =>
          //   between(this.today, this.twoWeeksFromToday)(new Date(value)),
          // $lazy: true,
        },
